...
mem = mem[mem.card_no.isin(cards.card_no)].merge(cards[["card_no", "giscode"]], on="card_no")
...
c = kh[~kh.exact].merge(
    mem[["giscode", "nk", "fk", "name_dev", "member_father_dev", "relationship_dev"]].rename(
        columns={"fk": "fk_card"}
    ),
    on=["giscode", "nk"],
    how="inner",
)
c["ratio"] = [difflib.SequenceMatcher(None, a, b).ratio() for a, b in zip(c.fk, c.fk_card)]
...
print(
    ...
    .sample(min(15, len(best[best.ratio >= 0.7])), random_state=1)[
        ["name", "relation", "relative", "member_father_dev", "relationship_dev", "ratio"]
    ]
    .to_string()
)
print(
    ...
    .sample(10, random_state=1)[
        ["name", "relation", "relative", "name_dev", "member_father_dev", "relationship_dev"]
    ]
    .to_string()
)
